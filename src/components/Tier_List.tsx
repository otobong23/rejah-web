...
const Tier_List: FC<Tier_List_Props> = ({
   ...
   handleBUY,
   bg= 'bg-[#040439]',
   btn_bg = 'bg-[linear-gradient(180deg,_#0000CD_0%,_#003B46_145.74%)]'
}) => {
   return (
         ...
         <div className="flex items-center gap-6 px-[15px] py-5 bg-[#fff] rounded-[13px]">
            ...
            <div className='flex-[1.5]'>
               ...
               {handleBUY && <div onClick={() => handleBUY({title, type, details})} className='flex justify-end mt-1.5'>
                  <button className={`text-sm text-[#FBFBFF] py-1.5 px-10 rounded-[10px] transition-all duration-300 hover:scale-90 ${btn_bg}`}>
                     Buy
                  </button>
               </div>}
            </div>
