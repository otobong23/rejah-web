...
const Vault_List: FC<Vault_List_Props> = ({
   ...
   return (
      <div className={`flex-1 text-(--color2) rounded-[15px] p-[3px] shadow-lg ${bg}`}>
         <h2 className="text-lg font-semibold px-[22px] py-[9px] text-[#E8E3D3]">{title}</h2>

         <div className="flex items-center gap-6 px-[15px] py-5 bg-(--background) rounded-xl">
               ...
               <table className="w-full text-sm text-left">
                  <tbody>
                     {Object.entries(details).map(([key, value]) => (
                        <tr key={title + '_' + key}>
                           <td className="px-[1px] py-[1px] text-(--color2)">{key.replace('_', ' ')}:</td>
